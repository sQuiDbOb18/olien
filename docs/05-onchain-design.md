...
| --- | --- | --- | --- |
| One-signature spending under a cap, per member, per period | Safe **Allowance module** (`allowances[safe][delegate][token]`: amount, spent, resetTimeMin, lastResetMin; `executeAllowanceTransfer` with the delegate's signature) | absent at `0xCFbFaC74C26F8647cBDb8c5caf80BB5b32E43134` | deploy from source at the canonical address through the singleton factory; verify bytecode hash matches the audited release |
